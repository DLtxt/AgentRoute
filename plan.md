...
**Status: complete. The labeling run finished (291 rows) and produced a negative result worth keeping — see below.**
...
- **Build the labeled dataset by outcome, not intuition.** Collect ~300 prompts, run each through all three tiers, and label each with *the cheapest tier that produced an acceptable answer*. About $3.90 in API spend. This is the difference between a real dataset and a model trained to predict your own guesses — an intuition-labeled model only learns to reproduce the rule classifier, so comparing the two would measure nothing.
  ...
  **Report the majority-class baseline next to every accuracy.** Without it, 0.685 on a dataset that is 70% one class reads as a mediocre result rather than a failing one. Report the ceiling too — the best achievable score given identical feature vectors — since the gap between baseline and ceiling is the only signal there is to learn.
...
- **Why XGBoost over logistic regression?** Neither beat a majority-class baseline on my dataset — 0.589 and 0.685 against 0.698 for always answering "haiku". The interesting part is why: my corpus was template-generated, so prompts differed by one noun and collided in feature space, and the judge labeled near-identical prompts inconsistently. 42% of rows had identical feature vectors with conflicting labels, which nothing can separate. The fix is the data, not the model. (Reporting a negative result you diagnosed beats a tuned number you can't explain.)
